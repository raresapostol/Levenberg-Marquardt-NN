...
#include "mathParameters.h"

class phiNNParameters : public mathParameters {
private:
protected:
public:
		...
		int outputLayerNode)
	{
		...
		this->phiNNTrainingNumber = this->inOutLengthNNDataset;

		// set the ptrNN parameters to use in other sections
		this->phiNNNumberOfHiddenLayerNode = hiddenLayerNode;
		this->phiNNNumberOfInputLayerNode = this->inOutNumberNNInputDataset;
		this->phiNNNumberOfOutputLayerNode = this->inOutNumberNNOutputDataset;
		...
		this->phiNNJacobianTerm = this->inOutCreatingEmptyMatrices(this->phiNNTrainingNumber,
	...
	double** phiNNPhiRandInitialization(double min, double max, int firstNodeNumber, int secondNodeNumber)
	{
		...
		double** pd = this->inOutCreatingEmptyMatrices(firstNodeNumber, secondNodeNumber);
	...
	void phiNNPhiTrainingNeuralNetworkWithLM()
	{
		...
		double** JacobianTermTrans = this->inOutCreatingEmptyMatrices((this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)),
			this->phiNNTrainingNumber);

		// multiplication of JacobianTerms
		double** JacobianMultiplicationResult = this->inOutCreatingEmptyMatrices(
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)),
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)));

		double** eyeMatrices = this->inOutCreatingEmptyMatrices(
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)),
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)));

		double** muEyeMatrices = this->inOutCreatingEmptyMatrices(
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)),
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)));

		double** JacobianFullTerm = this->inOutCreatingEmptyMatrices(
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)),
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)));

		double** InvJacobianFullTerm = this->inOutCreatingEmptyMatrices(
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)),
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)));

		double** coeffIntUpdate = this->inOutCreatingEmptyMatrices(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1),
			this->phiNNTrainingNumber);

		double** coeffUpdate = this->inOutCreatingEmptyMatrices(
			(this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)),
			1);
		...
		while (this->phiNNNnTrainingCondition != 0)
		{
			...
			// internal terms
			this->inOutPhiFree(coeffIntUpdate, this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1), this->phiNNTrainingNumber);
		...
		// LM update to coefficients
			...
			{
				this->inOutPhiFree(this->phiNNPhiWmatrices, this->phiNNH, this->phiNNI);
				this->inOutPhiFree(this->phiNNPhiVmatrices, this->phiNNK, this->phiNNH + 1);
				this->inOutPhiFree(this->phiNNErrorNow, this->phiNNTrainingNumber, this->phiNNK);
				this->inOutPhiFree(this->phiNNErrorPre, this->phiNNTrainingNumber, this->phiNNK);
				this->inOutPhiFree(this->phiNNErrorNowJac, this->phiNNTrainingNumber, this->phiNNK);

				this->inOutPhiFree(this->phiNNZActFunc, this->phiNNH + 1, this->phiNNTrainingNumber);
				this->inOutPhiFree(this->phiNNYActFunc, this->phiNNK, this->phiNNTrainingNumber);
		...
		for (int h = 0; h < this->phiNNH + 1; h++)
		{
			...
			else
			{
				...
				for (int i = 0; i < this->phiNNI; i++)
				{
					...
					internalSum = internalSum + this->phiNNPhiWmatrices[h][i] * this->inOutInputDataArray[i][index];
				}
			...
			for (int h = 0; h < this->phiNNH; h++)
			{
				for (int in = 0; in < this->phiNNI; in++)
				{
					this->phiNNJacobianTerm[index][jacobianCounter] = this->phiNNPhiVmatrices[k][h] *
						...
						this->inOutInputDataArray[in][index] *
						(-1.0);
	...
	void phiNNCostFunctionCalculation(int index)
	{
		...
		for (int k = 0; k < this->phiNNK; k++)
		{
			...
			this->phiNNErrorNow[index][k] = fabs(1.0 / 2 *
				(this->inOutOutputDataArray[k][index] - this->phiNNYActFunc[k][index]) *
				(this->inOutOutputDataArray[k][index] - this->phiNNYActFunc[k][index]));
		}
	}
	...
	void phiNNCostFunctionCalculationJacobian(int index)
	{
		...
		{
			// 1/2*(y_tr - y)^2
			this->phiNNErrorNowJac[index][k] = (this->inOutOutputDataArray[k][index] - this->phiNNYActFunc[k][index]);
		}
	}
	...
	void phiNNPhiExitNNLib()
	{
		...
		this->inOutPhiFree(this->phiNNPhiWmatrices, this->phiNNH, this->phiNNI);
		this->inOutPhiFree(this->phiNNPhiVmatrices, this->phiNNK, this->phiNNH + 1);
		this->inOutPhiFree(this->phiNNErrorNow, this->phiNNTrainingNumber, 1);
		this->inOutPhiFree(this->phiNNErrorPre, this->phiNNTrainingNumber, 1);
		this->inOutPhiFree(this->phiNNErrorNowJac, this->phiNNTrainingNumber, 1);
		this->inOutPhiFree(this->phiNNZActFunc, this->phiNNH + 1, this->phiNNTrainingNumber);
		this->inOutPhiFree(this->phiNNYActFunc, this->phiNNK, this->phiNNTrainingNumber);
		this->inOutPhiFree(this->phiNNJacobianTerm, this->phiNNTrainingNumber, (this->phiNNH * this->phiNNI + this->phiNNK * (this->phiNNH + 1)));
	}
